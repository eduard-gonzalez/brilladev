...
if ( ! class_exists( 'Thim_Breadcrumbs' ) ) {
	class Thim_Breadcrumbs {
		...
		public function render() {
			...
			$this->add_crumb( _x( 'Home', 'breadcrumb', 'eduma' ), home_url() );
		...
		protected function add_crumbs_404() {
			$this->add_crumb( __( 'Error 404', 'eduma' ) );
		}
		...
		protected function add_crumbs_search() {
			if ( is_search() ) {
				$this->add_crumb( sprintf( esc_html__( 'Search results for &ldquo;%s&rdquo;', 'eduma' ), get_search_query() ), remove_query_arg( 'paged' ) );
			}
		}
		...
		protected function add_crumbs_single( $post_id = 0, $permalink = '' ) {
			...
			if ( 'post' !== get_post_type( $post ) ) {
				...
				// get current category single post_type
		...
		protected function add_crumbs_post_type_archive() {
			...
			if ( $post_type ) {
				if ( get_post_type() == 'lp_course' ) {
					...
					$custom_name = apply_filters( 'thim_breadcrumb_post_type_name', '' );
				}
		...
		protected function add_crumbs_author() {
			...
			$this->add_crumb( sprintf( __( 'Author: %s', 'eduma' ), $userdata->display_name ) );
		}
		...
		protected function crumb_paged() {
			if ( get_query_var( 'paged' ) ) {
				$this->add_crumb( sprintf( esc_html__( 'Page %d', 'eduma' ), get_query_var( 'paged' ) ) );
			}
		}
	}
}
...
if ( ! function_exists( 'thim_breadcrumbs' ) ) {
	...
	function thim_breadcrumbs() {
		$breadcrumbs = new  Thim_Breadcrumbs();
