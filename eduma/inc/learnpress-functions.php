...
function thim_item_quiz_meta_questions( $item ) {
	...
	echo '<span class="meta count-questions">' . sprintf( $count ? _n( '%d preguntas', '%d preguntas', $count, 'eduma' ) : __( '%d preguntas', 'eduma' ), $count ) . '</span>';
}
...
if ( ! function_exists( 'thim_learnpress_count_lesson' ) ) {
	function thim_learnpress_count_lesson() {
		...
		if ( $course ) {
			...
			echo '<div class="course-lesson"><div class="value"><i aria-hidden="true" class="tk tk-file-invoice"></i>' . absint( $lessons ) . '</div></div>';
		}
	}
}
if ( ! thim_is_new_learnpress( '4.0.0' ) ) {
	add_filter('thim_core_can_update_theme', function (){
		return false;
	});
}
